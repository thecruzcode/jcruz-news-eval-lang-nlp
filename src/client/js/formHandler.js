function handleSubmit(event) {
  ...
  async function getData() {
    let response = await fetch('http://localhost:3030/test', {
      method: 'POST',
      headers: {
    ...
    try {
      ...
      document.querySelector('.polarity_confidence').textContent =
        newData.polarity_confidence;
      document.querySelector('.subjectivity_confidence').textContent =
